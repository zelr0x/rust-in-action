...
#[global_allocator] // Value marked with this satisfies GlobalAlloc trait.
...
// Contains data useful for the lifetime of the program.
struct World {
    current_turn: u64,
...
impl Particle {
    ...
    fn update(&mut self) {
        ...
        self.position = add(self.position,
                            self.velocity);
        // Slows down particle's rate of increase as it travels across the screen.
        self.acceleration = mul_scalar(
            self.acceleration,
            0.7
        );
        self.color[3] *= 0.995; // Make particle more transparent over time.
    }
}
...
impl World {
    fn new(width: f64, height: f64) -> World {
        World {
            current_turn: 0,
            // Boxed to add an extra allocation when particle is created
    ...
    fn remove_shapes(&mut self, n: i32) {
        for _ in 0..n.abs() {
            ...
            // For n iterations, removes the first invisible particle.
            // If there is none invisible - removes the oldest.
