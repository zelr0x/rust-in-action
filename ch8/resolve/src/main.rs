...
fn main() {
    ...
    let _amt = localhost.send_to(&req_b, dns_server)
        .expect("socket misconfigured");
    let (_amt, _remote) = localhost.recv_from(&mut resp_b)
        .expect("timeout_reached");
    let dns_message = Message::from_vec(&resp_b)
        .expect("unable to parse response");

    for ans in dns_message.answers() {
        if ans.record_type() == RecordType::A {
            let resource = ans.rdata();
